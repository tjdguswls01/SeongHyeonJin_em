...
//		while (XAxiVdma_ChannelIsRunning(&drv_inst_.WriteChannel)) ;
	...
	{
		configureRead(h_res, v_res, h_res, v_res);
	}

	void configureReadFixed(uint16_t h_res, uint16_t v_res, uint32_t read_base_addr)
	{
		XStatus status;
		...
		for (int iFrm=0; iFrm<drv_inst_.MaxNumFrames; ++iFrm) {
			context_.ReadCfg.FrameStoreStartAddr[iFrm] = read_base_addr;
	...
	void configureRead(uint16_t h_res, uint16_t v_res, uint16_t stride_pixels, uint16_t frame_store_v_res)
	{
		configureRead(h_res, v_res, stride_pixels, frame_store_v_res, 0, 0);
	}

	void configureRead(uint16_t h_res,
			uint16_t v_res,
			uint16_t stride_pixels,
			uint16_t frame_store_v_res,
			uint16_t start_x,
			uint16_t start_y)
	{
		XStatus status;
		context_.ReadCfg.HoriSizeInput = h_res * drv_inst_.ReadChannel.StreamWidth;
		context_.ReadCfg.VertSizeInput = v_res;
		context_.ReadCfg.Stride = stride_pixels * drv_inst_.ReadChannel.StreamWidth;
		context_.ReadCfg.FrameDelay = 1;
		context_.ReadCfg.EnableCircularBuf = 1;
		context_.ReadCfg.EnableSync = 1;
		context_.ReadCfg.PointNum = 0;
		...
		uint32_t read_offset = start_y * context_.ReadCfg.Stride + start_x * drv_inst_.ReadChannel.StreamWidth;
		for (int iFrm=0; iFrm<drv_inst_.MaxNumFrames; ++iFrm) {
			context_.ReadCfg.FrameStoreStartAddr[iFrm] = addr + read_offset;
			xil_printf("VDMA Frame %d Addr: 0x%08x\r\n", iFrm, addr + read_offset);
			//memset((void*)addr,0,context_.ReadCfg.HoriSizeInput * context_.ReadCfg.VertSizeInput);
			addr += context_.ReadCfg.Stride * frame_store_v_res;
		}
		status = XAxiVdma_DmaSetBufferAddr(&drv_inst_, XAXIVDMA_READ, context_.ReadCfg.FrameStoreStartAddr);
		if (XST_SUCCESS != status)
		{
