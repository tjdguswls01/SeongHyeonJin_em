...
timing_t const timing[] = {
		...
		{Resolution::R1024_600_60_PP, 1024, 40, 104, 144, timing_t::NEG, 600, 1, 3, 18, timing_t::POS, 48960000},
...
//		Configure video clock generator first, since losing clock will reset all IP connected to it
		...
		case 74250000:
			...
			break;
		case 48960000:
			//Factors for 244.8 MHz, 5x pixel clock for 1024x600 HDMI
			mul = 9.792; divclk = 1; clkout_div0 = 4.0;
			break;
		case 25000000:
			//Factors for 125 MHz
		...
		xil_printf("Video clock config: pclk=%lu Hz\r\n", (unsigned long)timing[i].pclk_freq_Hz);
		u32 lock_wait = 0;
		while (!(XClk_Wiz_ReadReg(sClkWiz_.Config.BaseAddr, 0x4) & 0x1)) {
			if ((lock_wait % 5000000U) == 0U) {
				xil_printf("Video clock waiting for lock...\r\n");
			}
			if (lock_wait++ > 50000000U) {
				xil_printf("Video clock lock timeout\r\n");
				break;
			}
		}
		if (XClk_Wiz_ReadReg(sClkWiz_.Config.BaseAddr, 0x4) & 0x1) {
			xil_printf("Video clock locked\r\n");
		}
